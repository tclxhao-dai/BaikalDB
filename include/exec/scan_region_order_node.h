...
namespace baikaldb {
class ScanRegionOrderNode : public ExecNode {
public:
    ScanRegionOrderNode() {
    }
    virtual ~ScanRegionOrderNode() {
    }
    ...
    int get_all_regions(RuntimeState* state);
    bool get_batch(RowBatch* batch);
    // fullexport分批获取region
    void get_next_partition(RuntimeState* state);
...
private:
    ...
    std::map<int64_t, int64_t> _region_id_sort;
